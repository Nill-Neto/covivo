...
CREATE OR REPLACE FUNCTION public.create_expense_with_splits(_group_id uuid, _title text, _description text DEFAULT NULL::text, _amount numeric DEFAULT 0, _category text DEFAULT 'other'::text, _expense_type text DEFAULT 'collective'::text, _due_date date DEFAULT NULL::date, _receipt_url text DEFAULT NULL::text, _recurring_expense_id uuid DEFAULT NULL::uuid, _target_user_id uuid DEFAULT NULL::uuid, _payment_method text DEFAULT 'cash'::text, _credit_card_id uuid DEFAULT NULL::uuid, _installments integer DEFAULT 1, _purchase_date date DEFAULT NULL::date, _participant_user_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _expense_type = 'individual' THEN
    -- Only create a split if the target is not the payer (e.g. admin creating for someone else)
    IF _effective_participants[1] <> _caller_id THEN
        INSERT INTO expense_splits (expense_id, user_id, amount, credor_user_id, group_id)
        VALUES (_expense_id, _effective_participants[1], _amount, _caller_id, _group_id);
    END IF;
  ELSE -- Collective
    ...
    IF _member_count > 0 THEN
      ...
      IF _group_rule = 'equal' OR array_length(_participant_user_ids, 1) IS NOT NULL THEN
        ...
        FOREACH _participant_id IN ARRAY _effective_participants LOOP
          IF _participant_id <> _caller_id THEN
            INSERT INTO expense_splits (expense_id, user_id, amount, credor_user_id, group_id)
            VALUES (_expense_id, _participant_id, _split_amount, _caller_id, _group_id);
          END IF;
        END LOOP;
      ELSE -- Proportional
        FOR _member IN
          ...
          WHERE gm.group_id = _group_id
            ...
            AND gm.user_id = ANY(_effective_participants)
        LOOP
          IF _member.user_id <> _caller_id THEN
            _split_amount := round(_amount * _member.pct / 100, 2);
            INSERT INTO expense_splits (expense_id, user_id, amount, credor_user_id, group_id)
            VALUES (_expense_id, _member.user_id, _split_amount, _caller_id, _group_id);
          END IF;
        END LOOP;
      END IF;
    END IF;
