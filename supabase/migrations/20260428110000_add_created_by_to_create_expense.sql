...
-- Recreate the function with an explicit _created_by parameter
CREATE OR REPLACE FUNCTION public.create_expense_with_splits(
  _group_id uuid,
  _created_by uuid, -- Added parameter
  _title text,
...
DECLARE
  _expense_id uuid;
  _caller_id uuid := COALESCE(_created_by, auth.uid()); -- Use passed ID or fallback to auth.uid()
  _member record;
