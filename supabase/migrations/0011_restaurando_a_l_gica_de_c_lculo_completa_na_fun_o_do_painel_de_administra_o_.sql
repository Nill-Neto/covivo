CREATE OR REPLACE FUNCTION public.fetch_admin_dashboard_metrics(
    _group_id uuid,
...
DECLARE
    result JSON;
BEGIN
    -- Authorization check: only admins of the group can run this.
    IF NOT public.has_role_in_group(auth.uid(), _group_id, 'admin') THEN
        RAISE EXCEPTION 'Unauthorized: Caller is not an admin of the specified group';
    END IF;
        ...
        'members', COALESCE((
            SELECT json_agg(
                json_build_object(
                    ...
                    'previous_debt', COALESCE(b.previous_debt, 0),
                    'current_cycle_owed', COALESCE(b.current_cycle_owed, 0),
                    'current_cycle_paid', COALESCE(b.current_cycle_paid, 0),
                    'accrued_debt', COALESCE(b.accrued_debt, 0)
                )
            )
            ...
            LEFT JOIN get_admin_member_competence_balances(_group_id, _competence_key) b ON b.user_id = m.user_id
